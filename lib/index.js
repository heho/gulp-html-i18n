(function() {
  ...
  handleUndefined = function(propName, opt) {
    if (opt.failOnMissing) {
      ...
      return log(colors.red(`${propName} not found in definition file!`));
    }
  };
  ...
  regexReplaceProperties = function(langRegExp, delimiters, content, properties, opt, lv) {
    ...
    if (opt.templateLiterals !== false) {
      content = content.replace(/"([^"]*)"/g, function(match, inner) {
        return /\$\{[^}]+\}/.test(inner) ? '`' + inner + '`' : match;
      });
      content = content.replace(/'([^']*)'/g, function(match, inner) {
        return /\$\{[^}]+\}/.test(inner) ? '`' + inner + '`' : match;
      });
